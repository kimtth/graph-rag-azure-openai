...
_ = load_dotenv(dotenv_path=env_file_path)

config_file_path = os.path.join(os.path.dirname(__file__), 'settings.yml')

settings = load_settings_from_yaml(config_file_path)
...
api_version = os.environ['AZURE_OPEN_AI_API_VERSION']
endpoint = os.environ['AZURE_OPEN_AI_ENDPOINT']
llm_model = os.environ['AZURE_OPEN_AI_DEPLOYMENT_MODEL']
embedding_model = os.environ['AZURE_OPEN_AI_DEPLOYMENT_EMBEDDING_MODEL']
...
def load_parquet_files():
    entity_df = pd.read_parquet(f"{INPUT_DIR}{os.path.sep}{ENTITY_TABLE}.parquet")
    entity_embedding_df = pd.read_parquet(f"{INPUT_DIR}{os.path.sep}{ENTITY_EMBEDDING_TABLE}.parquet")
    report_df = pd.read_parquet(f"{INPUT_DIR}{os.path.sep}{COMMUNITY_REPORT_TABLE}.parquet")
    relationship_df = pd.read_parquet(f"{INPUT_DIR}{os.path.sep}{RELATIONSHIP_TABLE}.parquet")    
    covariate_df = pd.read_parquet(f"{INPUT_DIR}{os.path.sep}{COVARIATE_TABLE}.parquet") if claim_extraction_enabled else pd.DataFrame()
    text_unit_df = pd.read_parquet(f"{INPUT_DIR}{os.path.sep}{TEXT_UNIT_TABLE}.parquet")
